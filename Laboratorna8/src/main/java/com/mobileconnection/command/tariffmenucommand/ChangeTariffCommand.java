...
public class ChangeTariffCommand implements Command {
    private static final Logger logger = LoggerFactory.getLogger(ChangeTariffCommand.class);
    @Override
    public void execute() {
        ...
        while(true) {
            ...
            for (int i = 0; i < fields.length; i++)
                try {
                    ...
                    logger.error("Помилка доступу!", e);
                }
            ...
            try {
                ...
                choice = -1;
                logger.error("Помилка введення даних!", e);
            }
            ...
            if (choice > 0 && choice <= fields.length) {
                ...
                logger.info("Поле \"" + fields[choice - 1].getName() + "\" тарифу \"" + getCurrentTariff().getName() + "\" змінено!");
            }
        }
        for (Field field : fields)
