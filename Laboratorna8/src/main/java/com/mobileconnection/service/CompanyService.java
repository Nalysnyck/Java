        ...
        for (int i = 0; i < filesNumber; ) {
            if (files[i].isFile()) {
                try {
                    ...
                    MyLogger.logger.severe("Помилка читання файлу " + files[i].getName());
                    return null;
                }
                i++;
            }
    ...
    public static void addCompany(MobileCompany mobileCompany){
        ...
        if(file.exists())
            ...
            try {
                ...
                MyLogger.logger.severe("Помилка при створенні файлу " + file.getName());
            }
        }
    }
    ...
    public static void overwriteCompany(MobileCompany company, String oldName){
        ...
        if(!oldFile.exists())
            ...
            if (!oldFile.delete()) {
                ...
                MyLogger.logger.severe("Помилка при видаленні файлу " + oldFile.getName());
            }
            else
                addCompany(company);
        }
    ...
    public static void deleteCompany(MobileCompany company){
        ...
        if(file.exists()) {
            if(!file.delete()) {
                ...
                MyLogger.logger.severe("Помилка при видаленні файлу " + file.getName());
            }
            else
